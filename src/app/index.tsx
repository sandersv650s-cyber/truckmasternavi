...
export default function Home() {
  ...
  const load = useCallback(async () => {
    ...
    try {
      ...
      const [cs, ms, msgs] = await Promise.all([
        ...
        client.from('messages').select('id,conversation_id,sender_id,original_text,original_language,created_at').in('conversation_id', ids).order('created_at', { ascending: false }).limit(200),
      ]);
      ...
      const latest = [...new Map([...((msgs.data ?? []).slice().reverse())].map(m => [m.conversation_id, m])).values()];
      const translationRows = latest.length && profile ? await client.from('message_translations').select('message_id,translated_text').in('message_id', latest.map(m => m.id)).eq('language', profile.language) : { data: [], error: null };
      const translated = new Map((translationRows.data ?? []).map(t => [t.message_id, t.translated_text]));
      setRows((cs.data ?? []).map(c => {
        ...
        return { id: c.id, name: c.title || others.map(p => p.display_name).join(', ') || 'Gesprek', others, last: last ? translated.get(last.id) ?? last.original_text : undefined, at: last?.created_at ?? c.created_at };
  ...
  useEffect(() => {
    if (!session) return;
    const client = requireClient();
    const channel = client.channel(`home:${session.user.id}`).on('postgres_changes', { event: 'INSERT', schema: 'public', table: 'messages' }, () => void load()).subscribe();
  ...
  return <ScrollView contentInsetAdjustmentBehavior="automatic" contentContainerStyle={{ padding: 20, gap: 16 }}>
    ...
    <Button title="＋  Nieuw gesprek of groep" onPress={() => router.push('/new')} />
